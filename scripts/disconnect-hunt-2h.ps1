...
.SYNOPSIS
  ...
  was covered by the AWS CIDR list. 2-hour version.
...
.PARAMETER EtlSizeMB
  Circular capture buffer size. Default 2048.

.PARAMETER KeepRaw
  Keep the decoded text file instead of deleting it.

.EXAMPLE
  .\disconnect-hunt-2h.ps1
.EXAMPLE
...
if ($freeGB -lt $needGB) {
  ...
  Write-Host "       e.g. -EtlSizeMB 512 needs about $([math]::Ceiling((512*9)/1024)) GB." -ForegroundColor Red
  exit 1
}
...
if ($KeepRaw) {
  ...
  Write-Host "Raw capture files deleted. Pass -KeepRaw to keep them."
}
